module ItemActions where
    ...
    take itemName state = case List.find (\x -> location (you state) == location x) (enemies state) of
        Nothing -> case List.find (\x -> itemName == Items.name x && location (you state) == itemLocation x) (items state) of 
        ...
        Just enemy -> state { comment = ["You cannot take items in room when there is " ++ Character.name enemy ++ " in it."]}

    drop :: String -> State -> State
    drop itemName state = case List.find (\x -> location (you state) == location x) (enemies state) of
        Nothing -> case List.find (\x -> itemName == Items.name x) (holding state) of
        ...
        Just enemy -> state { comment = ["You cannot drop items in room when there is " ++ Character.name enemy ++ " in it."]}

    inventory :: State -> State
    inventory state = state {comment = "Your inventory:" : map Items.name (holding state)}

    search :: State -> State
    search state = case List.find (\x -> location (you state) == location x) (enemies state) of
        Nothing -> case map Items.name (filter (\x -> itemLocation x == location (you state)) (items state)) of
            [] -> state { comment = ["There is nothing here"]}
            items -> state { comment = "You found these items:" : items }
        Just enemy -> state { comment = ["You cannot search this room when there is " ++ Character.name enemy ++ " in it."]}